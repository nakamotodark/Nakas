Nakas Core version 3.0.4 is now available from:

  <https://github.com/nakasproject/nakas/releases>
...
Nakas Core v3.0.4 is a mandatory update for all users. This release contains various updates/fixes pertaining to the zNAK protocol, supply tracking, block transmission and relaying, as well as usability and quality-of-life updates to the GUI.
...
If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/Nakas-Qt (on Mac) or nakasd/nakas-qt (on Linux).


Compatibility
==============

Nakas Core is extensively tested on multiple operating systems using
...
Nakas Core should also work on most other Unix-like systems but is not
...
- #294 `27c0943` Add additional checks for txid for znak spend. (presstab)
...
- #327 `aa1235a` [Main] Don't limit zNAK spends from getting into the mempool (Fuzzbawls)
...
### GUI
- #309 `f560ffc` [UI] Better error message when too much inputs are used for spending zNAK (Mrs-X)
...
- #314 `f1c830a` Fix issue causing crash when nakasd --help was invoked (Jon Spock)
- #326 `8b6a13e` Combine 2 LogPrintf statement to reduce debug.log clutter (Jon Spock)
- #328 `a6c18c8` [Main] Nakas not responding on user quitting app (Aaron Langford)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/nakas-project-translations/).
