...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "nakas.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "nakasd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the NAK and zNAK money supply statistics") + " " + _("on startup"));
    ...
    if (showDebug)
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in NAK/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
                                                                FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in NAK/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, nakas, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in NAK/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Nakas specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupznak=<n>", strprintf(_("Enable automatic wallet backups triggered after each zPhr minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-znakbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zNAK backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif
//    strUsage += "  -anonymizenakasamount=<n>     " + strprintf(_("Keep N NAK anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Nakas Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Nakas is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Nakas Core is shutting down."));
...
#endif
    // Make sure only a single Nakas process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Nakas Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Nakas specific: zerocoin and spork DB's
                ...
                // Nakas: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZNAKMinted();
                        RecalculateZNAKSpent();
                    }
                    RecalculateNAKSupply(1);
                }
                ...
                // Nakas: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Nakas Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Nakas Core to complete") << "\n";
    ...
    if (GetBoolArg("-usehd", DEFAULT_USE_HD_WALLET) && !pwalletMain->IsLocked() && DEFAULT_ENABLE_WARN_ENCRYPTHD) {
        ...
        zwalletMain = new CzNAKWallet(pwalletMain->strWalletFile);
        ...
        uiInterface.InitMessage(_("Syncing zNAK wallet..."));

        bool fEnableZPhrBackups = GetBoolArg("-backupznak", true);
...
//    }
//
//    nAnonymizeNakasAmount = GetArg("-anonymizenakasamount", 0);
//    if (nAnonymizeNakasAmount > 999999) nAnonymizeNakasAmount = 999999;
//    if (nAnonymizeNakasAmount < 2) nAnonymizeNakasAmount = 2;
    ...
    LogPrintf("Anonymize Nakas Amount %d\n", nAnonymizeNakasAmount);
    ...
    /* Denominations
       ...
       For example:
       1NAK+1000 == (.1NAK+100)*10
       10NAK+10000 == (1NAK+1000)*10
    */
