...
public:
    ...
    bool WriteZNAKSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZNAKSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZNAKSeed_deprecated(uint256& seed);
    bool EraseZNAKSeed();
    bool EraseZNAKSeed_deprecated();

    bool WriteZNAKCount(const uint32_t& nCount);
    bool ReadZNAKCount(uint32_t& nCount);
