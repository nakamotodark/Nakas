...
#ifndef NAKAS_ZNAKWALLET_H
#define NAKAS_ZNAKWALLET_H
...
public:
    CzNAKWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZNAK(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZNAK(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //NAKAS_ZNAKWALLET_H
