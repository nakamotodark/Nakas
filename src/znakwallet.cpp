...
CzNAKWallet::CzNAKWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing znak seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNAKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNAKSeed_deprecated()) {
                    LogPrintf("%s: Updated zNAK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old znak seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of znak wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNAKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNAKCount(nCountLastUsed);
    else if (!walletdb.ReadZNAKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNAKWallet::Lock()
{
    seedMaster = 0;
}

void CzNAKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNAKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNAK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNAKWallet::LoadMintPoolFromDB()
{
...
void CzNAKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNAKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNAKWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->znakTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNAKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNAK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to znakTracker which also adds to database
    pwalletMain->znakTracker->Add(dMint, true);
...
void CzNAKWallet::SeedToZNAK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNAKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNAKWallet::GenerateDeterministicZNAK(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNAKWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNAKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
