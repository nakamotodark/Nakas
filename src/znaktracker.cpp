...
CzNAKTracker::CzNAKTracker(std::string strWalletFile)
{
...
CzNAKTracker::~CzNAKTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNAKTracker::Init()
{
...
bool CzNAKTracker::Archive(CMintMeta& meta)
{
...
bool CzNAKTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNAKTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNAKTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNAKTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzNAKTracker::GetSerialHashes()
{
...
CAmount CzNAKTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNAKTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNAKTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNAKTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNAKTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNAKTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNAKTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNAKTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNAKTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNAKTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNAKTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNAKTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNAKTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNAKTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNAKTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNAKTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNAKTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dznak from DB\n", __func__, listDeterministicDB.size());
    }
