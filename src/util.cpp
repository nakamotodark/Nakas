...
int nAnonymizeNakasAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "nakas" is a composite category enabling all Nakas-related debug output
            if (ptrCategory->count(string("nakas"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Nakas
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Nakas
// Mac: ~/Library/Application Support/Nakas
// Unix: ~/.nakas
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nakas";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "nakas.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty nakas.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override nakas.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "nakasd.pid"));
