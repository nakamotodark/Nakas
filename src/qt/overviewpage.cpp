...
// Copyright (c) 2017-2019 The Nakas Developers
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::NAK)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sNAKPercentage, QString& szNAKPercentage)
{
    int nPrecision = 2;
    ...
    szNAKPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sNAKPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount nakAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = nakAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // NAK labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, nakAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelNAKPercent->setText(sPercentage);
    ui->labelzNAKPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zNAK.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in nakas.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in nakas.conf");
    }
    ...
    bool showNAKAvailable = settingShowAllBalances || nakAvailableBalance != nTotalBalance;
    bool showWatchOnlyNAKAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showNAKPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyNAKPending = watchUnconfBalance != 0;
    bool showNAKLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyNAKLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showNAKAvailable || showWatchOnlyNAKAvailable);
    ui->labelBalanceText->setVisible(showNAKAvailable || showWatchOnlyNAKAvailable);
    ui->labelWatchAvailable->setVisible(showNAKAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showNAKPending || showWatchOnlyNAKPending);
    ui->labelPendingText->setVisible(showNAKPending || showWatchOnlyNAKPending);
    ui->labelWatchPending->setVisible(showNAKPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showNAKLocked || showWatchOnlyNAKLocked);
    ui->labelLockedBalanceText->setVisible(showNAKLocked || showWatchOnlyNAKLocked);
    ui->labelWatchLocked->setVisible(showNAKLocked && showWatchOnly);
    ...
    bool showzNAKAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzNAKUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzNAKImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzNAKAvailable);
    ui->labelzBalanceMatureText->setVisible(showzNAKAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzNAKUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzNAKUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzNAKImmature);
    ui->labelzBalanceImmatureText->setVisible(showzNAKImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelNAKPercent->setVisible(showPercentages);
    ui->labelzNAKPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("NAK")
    updateDisplayUnit();
}
