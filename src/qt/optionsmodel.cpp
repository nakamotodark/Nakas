...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NAK);
    ...
    if (!settings.contains("nAnonymizeNakasAmount"))
        settings.setValue("nAnonymizeNakasAmount", 1000);

    nAnonymizeNakasAmount = settings.value("nAnonymizeNakasAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNakasAmount"))
        SoftSetArg("-anonymizenakasamount", settings.value("nAnonymizeNakasAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nakas.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNakasAmount:
            return QVariant(nAnonymizeNakasAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNakasAmount:
            nAnonymizeNakasAmount = value.toInt();
            settings.setValue("nAnonymizeNakasAmount", nAnonymizeNakasAmount);
            emit anonymizeNakasAmountChanged(nAnonymizeNakasAmount);
            break;
        case CoinControlFeatures:
