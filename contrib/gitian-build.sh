...
VERSION=
commit=false
url=https://github.com/nakasproject/nakas
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the nakas, gitian-builder, gitian.sigs, and nakas-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/nakasproject/nakas
...
then
	# Make output folder
	mkdir -p "./nakas-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit nakas=${COMMIT} --url nakas=${url} ../nakas/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../nakas/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/nakas-*.tar.gz build/out/src/nakas-*.tar.gz ../nakas-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit nakas=${COMMIT} --url nakas=${url} ../nakas/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/nakas-*.zip build/out/nakas-*.exe ../nakas-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit nakas=${COMMIT} --url nakas=${url} ../nakas/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/nakas-*.tar.gz build/out/nakas-*.dmg ../nakas-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit nakas=${COMMIT} --url nakas=${url} ../nakas/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../nakas/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/nakas-*.tar.gz build/out/src/nakas-*.tar.gz ../nakas-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../nakas/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/nakas-*win64-setup.exe ../nakas-binaries/${VERSION}
	    mv build/out/nakas-*win32-setup.exe ../nakas-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../nakas/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/nakas-osx-signed.dmg ../nakas-binaries/${VERSION}/nakas-${VERSION}-osx.dmg
	fi
	popd || exit
